...
def load_leakdb_data(zip_path, scenarios=None):
    ...
    if not os.path.exists(zip_path):
        raise FileNotFoundError(f"ZIP file not found: {zip_path}\nPlease ensure the LeakDB.zip file exists at this path.")
    try:
        z = zipfile.ZipFile(zip_path)
    except zipfile.BadZipFile:
        raise ValueError(f"Invalid ZIP file: {zip_path}\nThe file may be corrupted or not a valid ZIP archive.")
    except Exception as e:
        raise RuntimeError(f"Error opening ZIP file {zip_path}: {e}")
    ...
    # Combine all data
    if not all_data:
        raise ValueError("No data loaded from zip file! Please check:\n"
                        "1. The ZIP file contains LeakDB/Hanoi_CMH/Scenario-*/Demands/ files\n"
                        "2. The file structure matches the expected format\n"
                        "3. CSV files are not corrupted")
    
    try:
        df = pd.concat(all_data, ignore_index=True)
    except Exception as e:
        raise ValueError(f"Error combining data from zip file: {e}\n"
                        "This may indicate corrupted CSV files or incompatible data formats.")
...
def create_pipe_network_graph(df, connectivity_threshold=0.25):
    ...
    # Ensure we have at least some edges for graph connectivity
    if not edges and len(sensors) > 1:
        print("Warning: No edges created from connectivity rules. Creating minimum connectivity...")
        # Create a simple chain or star topology
        for i in range(len(sensors) - 1):
            edges.append([i, i + 1])
            edge_weights.append(1.0)
        # Also connect first and last if more than 2 nodes
        if len(sensors) > 2:
            edges.append([0, len(sensors) - 1])
            edge_weights.append(1.0)
    ...
    if edge_index.shape[1] == 0:
        print(f"Warning: Graph has no edges! This may cause issues with GNN training.")
        print(f"Consider adjusting connectivity_threshold (current: {connectivity_threshold})")
    
    return edge_index, edge_attr, sensor_to_idx, positions

def create_temporal_graph_data(df, edge_index, edge_attr, sensor_to_idx, target_shift_steps=0, target_col="Leak Status"):
    """
...
if len(df) < initial_rows:
    print(f"Warning: Removed {initial_rows - len(df)} rows with invalid timestamps")

if len(df) == 0:
    raise ValueError("No valid data remaining after timestamp processing! Check your data files.")
...
graph_sequence, timestamps = create_temporal_graph_data(df, edge_index, edge_attr, sensor_to_idx, target_shift_steps, target_col)
...
if len(graph_sequence) > 0:
    ...
    try:
        ...
        print(f"Warning: Could not save graph sequence: {e}")
...
graph_sequence_train, timestamps_train = create_temporal_graph_data(
    df_train, edge_index, edge_attr, sensor_to_idx, target_shift_steps, target_col
) if len(df_train) > 0 else ([], [])
graph_sequence_val, timestamps_val = create_temporal_graph_data(
    df_val, edge_index, edge_attr, sensor_to_idx, target_shift_steps, target_col
) if len(df_val) > 0 else ([], [])
graph_sequence_test, timestamps_test = create_temporal_graph_data(
    df_test, edge_index, edge_attr, sensor_to_idx, target_shift_steps, target_col
) if len(df_test) > 0 else ([], [])
...
if len(graph_sequence_train) > 0:
    ...
    # Create a dummy scaler if no training data
    scaler = StandardScaler()
...
if len(graph_sequence_train) == 0:
    ...
    print("1. No data in training split")
    print("2. Data filtering removed all training samples")
    print("3. Graph creation failed")
    print(f"Total graphs available: train={len(graph_sequence_train)}, val={len(graph_sequence_val)}, test={len(graph_sequence_test)}")
    if len(graph_sequence) > 0:
        print(f"Full graph sequence has {len(graph_sequence)} graphs - consider using all data for training")
    exit(1)
...
# -------------------------------------------------------------
# 7. Graph Visualization - ALWAYS PRODUCE VISUAL GRAPH WITH NODES AND EDGES
# -------------------------------------------------------------
print("\n=== Creating Graph Visualization ===")

# Ensure we have graph data to visualize
graph_to_visualize = None
if len(graph_sequence_train) > 0:
    graph_to_visualize = graph_sequence_train[0]
elif len(graph_sequence) > 0:
    graph_to_visualize = graph_sequence[0]
elif len(graph_sequence_val) > 0:
    graph_to_visualize = graph_sequence_val[0]
elif len(graph_sequence_test) > 0:
    graph_to_visualize = graph_sequence_test[0]

if graph_to_visualize is not None:
    try:
        print(f"Using graph with {graph_to_visualize.x.shape[0]} nodes and {graph_to_visualize.edge_index.shape[1]} edges")
        
        # Convert to NetworkX for visualization
        G = to_networkx(graph_to_visualize, to_undirected=True, remove_self_loops=True)
        
        print(f"NetworkX graph created: {G.number_of_nodes()} nodes and {G.number_of_edges()} edges")
        
        # If graph is empty, create edges from edge_index directly
        if G.number_of_edges() == 0 and graph_to_visualize.edge_index.numel() > 0:
            print("NetworkX conversion produced no edges, creating graph from edge_index...")
            G = nx.Graph()
            edge_list = graph_to_visualize.edge_index.t().numpy()
            G.add_edges_from(edge_list)
            print(f"Graph created from edge_index: {G.number_of_nodes()} nodes and {G.number_of_edges()} edges")
        
        # Ensure we have nodes
        if G.number_of_nodes() == 0:
            # Create nodes from sensor_to_idx
            G.add_nodes_from(range(len(sensor_to_idx)))
            print(f"Added {len(sensor_to_idx)} nodes to graph")
        
        # Determine if we need to sample for very large graphs
        max_nodes_for_detailed = 150
        original_G = G.copy()
        if G.number_of_nodes() > max_nodes_for_detailed:
            print(f"Graph is large ({G.number_of_nodes()} nodes). Creating sampled visualization...")
            # Sample nodes: keep nodes with highest degree (most connected)
            degrees = dict(G.degree())
            if len(degrees) > 0:
                top_nodes = sorted(degrees.items(), key=lambda x: x[1], reverse=True)[:max_nodes_for_detailed]
                nodes_to_keep = [node for node, _ in top_nodes]
                # Also keep neighbors of top nodes
                for node in nodes_to_keep[:20]:  # Keep neighbors of top 20 nodes
                    nodes_to_keep.extend(list(G.neighbors(node)))
                nodes_to_keep = list(set(nodes_to_keep))[:max_nodes_for_detailed]
                G = G.subgraph(nodes_to_keep).copy()
                print(f"Sampled to {G.number_of_nodes()} nodes for visualization")
        
        # Create figure with better size
        fig_size = (16, 12) if G.number_of_nodes() > 50 else (12, 10)
        ...
        # Color nodes by leak status - create proper mapping
        node_colors_dict = {}
        node_sizes_dict = {}
        node_labels = {}
        
        # Map node indices back to sensor IDs if possible
        ...
        for node_idx in node_list:
            ...
            is_active = False
            try:
                if hasattr(graph_to_visualize, 'sensor_mask') and graph_to_visualize.sensor_mask is not None:
                    if graph_to_visualize.sensor_mask.numel() > node_idx:
                        is_active = bool(graph_to_visualize.sensor_mask[node_idx].item())
                if hasattr(graph_to_visualize, 'y') and graph_to_visualize.y is not None:
                    if graph_to_visualize.y.numel() > node_idx:
                        leak_status = float(graph_to_visualize.y[node_idx].item())
            except (IndexError, AttributeError, RuntimeError) as e:
                # If we can't access the attributes, assume inactive
                is_active = False
                leak_status = 0.0
            
            # Set color and size based on leak status
            if is_active and leak_status > 0.5:
                node_colors_dict[node_idx] = 'red'
                node_sizes_dict[node_idx] = 800
            elif is_active:
                node_colors_dict[node_idx] = 'lightblue'
                node_sizes_dict[node_idx] = 500
            else:
                node_colors_dict[node_idx] = 'gray'
                node_sizes_dict[node_idx] = 300
            
            # Create labels
            if node_idx in idx_to_sensor:
                sensor_id = idx_to_sensor[node_idx]
                # Shorten label for readability
                if '_Node_' in sensor_id:
                    label = sensor_id.split('_Node_')[1]
                else:
                    label = str(node_idx)
                node_labels[node_idx] = label
            else:
                node_labels[node_idx] = str(node_idx)
        
        # Convert to lists in node order
        node_colors = [node_colors_dict.get(node, 'gray') for node in node_list]
        node_sizes = [node_sizes_dict.get(node, 300) for node in node_list]
        
        # Draw edges first (so they're behind nodes) - ALWAYS DRAW EDGES
        if G.number_of_edges() > 0:
            nx.draw_networkx_edges(G, pos, alpha=0.4, width=2.0, edge_color='darkgray', style='solid')
            print(f"Drew {G.number_of_edges()} edges")
        else:
            # If no edges in G, draw from edge_index
            if graph_to_visualize.edge_index.numel() > 0:
                edge_list = graph_to_visualize.edge_index.t().numpy()
                for edge in edge_list[:min(1000, len(edge_list))]:  # Limit for performance
                    src, dst = edge[0], edge[1]
                    if src in pos and dst in pos:
                        plt.plot([pos[src][0], pos[dst][0]], [pos[src][1], pos[dst][1]], 
                                'gray', alpha=0.3, linewidth=1.5)
                print(f"Drew {min(1000, len(edge_list))} edges from edge_index")
        
        # Draw nodes with colors and sizes - ALWAYS DRAW NODES
        nx.draw_networkx_nodes(G, pos, node_color=node_colors, node_size=node_sizes, alpha=0.9)
        print(f"Drew {len(node_list)} nodes")
        
        # Draw labels (only for smaller graphs or important nodes)
        if G.number_of_nodes() <= 50:
            nx.draw_networkx_labels(G, pos, node_labels, font_size=8, font_weight='bold')
        else:
            # Only label nodes with leaks or high degree
            important_labels = {}
            node_list_sorted = list(G.nodes())
            for node in node_list_sorted:
                node_idx_in_list = node_list_sorted.index(node)
                if node_idx_in_list < len(node_colors) and node_colors[node_idx_in_list] == 'red':
                    important_labels[node] = node_labels.get(node, str(node))
                elif G.degree(node) >= np.percentile([G.degree(n) for n in G.nodes()], 75):
                    important_labels[node] = node_labels.get(node, str(node))
            if important_labels:
                nx.draw_networkx_labels(G, pos, important_labels, font_size=7, font_weight='bold')
        
        # Add legend
        from matplotlib.patches import Patch
        legend_elements = [
            Patch(facecolor='red', label='Leak Detected'),
            Patch(facecolor='lightblue', label='Normal'),
            Patch(facecolor='gray', label='Inactive/No Data')
        ]
        plt.legend(handles=legend_elements, loc='upper right', fontsize=10)
        
        plt.title(f"Water Distribution Network Graph Structure\n"
                 f"Nodes: {G.number_of_nodes()}, Edges: {G.number_of_edges()}\n"
                 f"(Red=Leak, Blue=Normal, Gray=Inactive)", 
                 fontsize=14, fontweight='bold')
        plt.axis('off')
        plt.tight_layout()
        
        # Save visualization to file
        viz_filename = "gnn_graph_visualization.png"
        plt.savefig(viz_filename, dpi=300, bbox_inches='tight')
        print(f"✓ Graph visualization saved to '{viz_filename}'")
        
        # Also show interactively
        plt.show()
        
        # Create a second visualization showing edge weights/strength
        if (hasattr(graph_to_visualize, 'edge_attr') and graph_to_visualize.edge_attr is not None and 
            graph_to_visualize.edge_attr.numel() > 0 and G.number_of_edges() > 0):
            plt.figure(figsize=fig_size)
            
            # Get edge weights for visualization
            edge_weights_list = []
            edge_list_nx = list(G.edges())
            for edge in edge_list_nx:
                src, dst = edge
                edge_found = False
                # Find edge in original edge_index
                if graph_to_visualize.edge_index.numel() > 0:
                    edge_list = graph_to_visualize.edge_index.t().numpy()
                    for e_idx, (e_src, e_dst) in enumerate(edge_list):
                        if (e_src == src and e_dst == dst) or (e_src == dst and e_dst == src):
                            weight = 1.0
                            try:
                                if (hasattr(graph_to_visualize, 'edge_attr') and 
                                    graph_to_visualize.edge_attr is not None and 
                                    graph_to_visualize.edge_attr.numel() > 0):
                                    if graph_to_visualize.edge_attr.dim() > 1 and e_idx < graph_to_visualize.edge_attr.shape[0]:
                                        weight = float(graph_to_visualize.edge_attr[e_idx, 0].item())
                                    elif e_idx < graph_to_visualize.edge_attr.numel():
                                        weight = float(graph_to_visualize.edge_attr[e_idx].item())
            ...
            if edge_weights_list and max(edge_weights_list) > 0:
                max_weight = max(edge_weights_list)
                edge_weights_normalized = [max(0.5, w / max_weight * 3.0) for w in edge_weights_list]
            else:
                edge_weights_normalized = [1.5] * len(edge_list_nx) if edge_list_nx else [1.5]
            
            # Draw with edge weights
            if G.number_of_edges() > 0:
                nx.draw_networkx_edges(G, pos, alpha=0.5, width=edge_weights_normalized, 
            ...
            plt.title(f"Water Distribution Network - Edge Strengths\n"
                     f"(Thicker edges = stronger connections)", 
                     fontsize=14, fontweight='bold')
            plt.axis('off')
            plt.tight_layout()
            
            viz_filename2 = "gnn_graph_visualization_edge_weights.png"
            plt.savefig(viz_filename2, dpi=300, bbox_inches='tight')
            print(f"✓ Edge weights visualization saved to '{viz_filename2}'")
            plt.show()
        
        print("✓ Graph visualization complete! Check the PNG files for visual output.")
        
    except Exception as e:
        print(f"Error creating graph visualization: {e}")
        import traceback
        traceback.print_exc()
        # Fallback: create a simple visualization using edge_index directly
        try:
            print("\nAttempting fallback visualization from edge_index...")
            plt.figure(figsize=(12, 10))
            
            if edge_index.numel() > 0:
                # Create a simple graph from edge_index
                edge_list = edge_index.t().numpy()
                
                # Create positions using spring layout
                num_nodes = len(sensor_to_idx)
                G_fallback = nx.Graph()
                G_fallback.add_nodes_from(range(num_nodes))
                G_fallback.add_edges_from(edge_list[:min(500, len(edge_list))])
                
                pos_fallback = nx.spring_layout(G_fallback, k=1, iterations=50, seed=42)
                
                # Draw edges
                for edge in edge_list[:min(500, len(edge_list))]:
                    src, dst = edge[0], edge[1]
                    if src in pos_fallback and dst in pos_fallback:
                        plt.plot([pos_fallback[src][0], pos_fallback[dst][0]], 
                                [pos_fallback[src][1], pos_fallback[dst][1]], 
                                'b-', alpha=0.3, linewidth=1.0)
                
                # Draw nodes
                for node in range(num_nodes):
                    if node in pos_fallback:
                        plt.plot(pos_fallback[node][0], pos_fallback[node][1], 
                                'ro', markersize=10, alpha=0.7)
                
                plt.title(f"Water Distribution Network Graph (Fallback)\n"
                         f"{num_nodes} nodes, {min(500, len(edge_list))} edges shown", 
                         fontsize=12, fontweight='bold')
                plt.axis('equal')
                plt.grid(True, alpha=0.3)
                plt.tight_layout()
                
                fallback_filename = "gnn_graph_visualization_fallback.png"
                plt.savefig(fallback_filename, dpi=200, bbox_inches='tight')
                print(f"✓ Fallback visualization saved to '{fallback_filename}'")
                plt.show()
    ...
    print("Available graph sequences:")
    print(f"  - graph_sequence_train: {len(graph_sequence_train)} graphs")
    print(f"  - graph_sequence_val: {len(graph_sequence_val)} graphs")
    print(f"  - graph_sequence_test: {len(graph_sequence_test)} graphs")
    print(f"  - graph_sequence: {len(graph_sequence) if 'graph_sequence' in locals() else 0} graphs")
...
if 'model' in locals() and model is not None:
    ...
    if 'input_dim' in locals():
        save_dict['input_dim'] = input_dim
    
    torch.save(save_dict, model_save_path)
    print(f"\nGNN model saved successfully as: {model_save_path}")
else:
    print("\n⚠ Model not available for saving (training may have failed)")
